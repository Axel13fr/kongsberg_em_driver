...
bool read_tx_sector_struct(uint8_t *ptr, int &count, const int max_length, txSectorInfo_S &s, const size_t numBytesPerTxSector)
{
...
static std::pair<bool, EMdgmMRZ_S> read_mrz(uint8_t *ptr, int max_length)
{
  ...
  if (mrz.pingInfo.numTxSectors > MAX_NUM_TX_PULSES)
  {
    ROS_ERROR_STREAM("Read " << mrz.pingInfo.numTxSectors << " sectors vs maximum of " << MAX_NUM_TX_PULSES);
    return error;
  }
  ...
  for (int i = 0; i < mrz.pingInfo.numTxSectors; i++)
  {
    ...
    {
      ROS_ERROR_STREAM("Failed decoding sector " << i << "/" <<mrz.pingInfo.numTxSectors <<  " from ping count " << mrz.cmnPart.pingCnt);
      return error;
    }
  }
  ...
  if (mrz.rxInfo.numExtraDetectionClasses > MAX_EXTRA_DET_CLASSES)
  {
    ROS_ERROR_STREAM("Read " << mrz.rxInfo.numExtraDetectionClasses << " sectors vs maximum of " << MAX_EXTRA_DET_CLASSES);
    return error;
  }
  ...
  if(nb_soundings > (MAX_NUM_BEAMS + MAX_EXTRA_DET)){
    ROS_ERROR_STREAM("Read " << nb_soundings << " sectors vs maximum of " << MAX_EXTRA_DET_CLASSES);
  }
  ...
  ROS_ERROR_STREAM_COND(mrz.header.numBytesDgm != check_length,"Header len: " << mrz.header.numBytesDgm << " Check len: "<<check_length);
  return {true, mrz};
}
